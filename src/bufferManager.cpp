...
void BufferManager::reloadPage(string tableName, int pageIndex)
{
    logger.log("BufferManager::reloadPage");
    // Remove the page from the pool if it exists
    string pageName = "../data/temp/"+tableName + "_Page" + to_string(pageIndex);
    if(this->inPool(pageName))
    {
        // Remove the page from the pool and insert it back
