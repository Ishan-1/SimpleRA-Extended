...
bool SecondaryIndex::updateIndex(int newValue)
{
  ...
  // Sort the index entries to maintain order for binary search
  sort(indexEntries.begin(), indexEntries.end(), [](const IndexEntry &a, const IndexEntry &b) {
    return a.fieldValue < b.fieldValue;
  });

  // Find the appropriate index block to update
  const int MAX_DISTINCT_VALUES_PER_INDEX_BLOCK = 100;
  string indexDir = "../data/indices/";
  bool valueAdded = false;
  int blockIndex = 0;
  
  // Try to find an existing index block with space
  while (!valueAdded)
  {
    string indexFileName = indexDir + tableName + "_" + columnName + "_Indexfile_" + to_string(blockIndex);
    ifstream checkFile(indexFileName);
    
    if (!checkFile) // File doesn't exist, create a new one
    {
      ofstream newIndexFile(indexFileName);
      if (!newIndexFile)
      {
        logger.log("SecondaryIndex::updateIndex: Failed to create new index file " + indexFileName);
        return false;
      }
      
      // Write 1 as the count (just this new value)
      newIndexFile << 1 << endl;
      
      // Write the new entry
      newIndexFile << newValue << " " << bpFileName << endl;
    ...
    else
    {
      ...
      if (distinctCount < MAX_DISTINCT_VALUES_PER_INDEX_BLOCK)
      {
        checkFile.close();
        
        // Read all entries into memory
        vector<pair<int, string>> entries;
        ifstream readFile(indexFileName);
        readFile >> distinctCount; // Skip count
        
        string line;
        getline(readFile, line); // Skip the rest of the first line
        ...
        // Add our new entry
        entries.push_back({newValue, bpFileName});
        ...
        if (!writeFile)
        {
          logger.log("SecondaryIndex::updateIndex: Failed to update index file " + indexFileName);
          return false;
        }
        
        // Update count
        writeFile << distinctCount + 1 << endl;
      ...
      {
        // This block is full, try the next one
        checkFile.close();
        blockIndex++;
      }
    }
